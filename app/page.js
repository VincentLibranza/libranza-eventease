...
export default function EventEaseApp() {
  ...
  // --- UPDATED EXCEL EXPORT (INCLUDES EMAIL & STATUS) ---
  const exportExcel = () => {
    ...
    const dataToExport = filtered.map(p => ({
      ...
      'Email': p.email,
      'Department': p.dept,
      'Status': p.status === 'CHECKED IN' ? 'Checked In' : 'Registered Only'
    }));
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] flex">
      ...
      <main className="ml-64 p-12 w-full">
        ...
        {view === 'dashboard' && (
          <div className="space-y-6 animate-in fade-in">
            <div className="grid grid-cols-4 gap-6">
              {[
                { l: 'Total Events', v: events.length, i: '📅', c: 'text-blue-600 border-blue-100 bg-blue-50/20' },
                { l: 'Total Participants', v: participants.length, i: '👥', c: 'text-emerald-600 border-emerald-100 bg-emerald-50/20' },
            ...
            <div className="p-10 rounded-[32px] text-white flex flex-col gap-4 shadow-xl" style={{ backgroundColor: primaryColor }}>
               <h3 className="text-xl font-black">📈 AI Prediction</h3>
               {!predictionResult ? (
                 <button onClick={() => alert("AI logic here")} className="bg-white text-indigo-700 px-8 py-3 rounded-2xl font-black w-fit">Generate Prediction</button>
               ) : (
                 <div className="bg-white/10 p-6 rounded-2xl font-black">Turnout Prediction: {predictionResult?.estimatedTurnout}</div>
               )}
            </div>
          </div>
        )}
        ...
        {/* 🛑 ATTENDANCE VIEW - REVERTED UI + EXCEL ENHANCED */}
        {view === 'attendance' && (
                   ...
                   <table className="w-full text-left font-black">
                      ...
                      <tbody>{participants.filter(p => p.eventId === selectedEventId).map(p => (
                        <tr key={p.id} className="border-b">
                          <td className="p-6 px-10 text-gray-900 text-lg">{p.name}</td>
                          <td className="p-6 px-10 text-right">
                             <button 
                               onClick={()=>handleCheckIn(p.id)} 
                               disabled={p.status==='CHECKED IN'} 
                               className={`px-8 py-2.5 rounded-2xl font-black text-sm transition-all ${p.status === 'CHECKED IN' ? 'bg-gray-200 text-gray-400' : 'bg-indigo-600 text-white shadow-lg'}`}
                             >
